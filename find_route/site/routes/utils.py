...
def get_routes(request, form) -> dict:
    ...
    if not len(all_ways):
        # нет ни одного маршрута для поиска
    ...
    if cities:
        ...
        # если города, через которые нужно проехать не заданы
        right_ways = all_ways
    ...
    for route in right_ways:
        ...
        if total_time <= traveling_time:
            # если время в пути, меньше заданного, то добавляем маршрут в общий список
            routes.append(tmp)

    if not routes:
        # если список пуст, то нет таких маршрутов, которые удовлетворяли бы заданные условия
